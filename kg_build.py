...
EXPERIMENTAL_CSV = PROJECT_ROOT / "data" / "processed" / "data_end.csv"
DENOVO_CSV = PROJECT_ROOT / "data" / "processed" / "DeNovo_Molecule.csv"
BATCH_SIZE = 1000

def resolve_domain_config_path() -> Path:
	candidates = [
		PROJECT_ROOT / "domain_config.json",
		PROJECT_ROOT / "src" / "config" / "domain_config.json",
		PROJECT_ROOT.parent / "domain_config.json",
	]
	for candidate in candidates:
		if candidate.exists():
			return candidate
	raise FileNotFoundError("domain_config.json was not found in any expected location")
...
class KnowledgeGraphBuilder:
	def __init__(
		...
		if config_path:
			...
			resolved = resolve_domain_config_path()
			self.analyzer = ChemistryAnalyzer.from_json(resolved)
			print(f"Loaded domain config: {resolved.name}")
